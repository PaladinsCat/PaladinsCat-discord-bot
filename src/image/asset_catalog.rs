...
impl AssetCatalog {
    ...
    pub fn champion_icon(&self, champion_name: &str) -> Option<PathBuf> {
        ...
        let result = files.iter().find(|f| f.file_stem().map(|s| normalized(&s.to_string_lossy()) == wanted).unwrap_or(false))
            .or_else(|| files.iter().find(|f| {
                let n = normalized(&f.to_string_lossy());
                n.contains(&normalized(champion_name)) && n.contains("icon")
            })).cloned();
        self.champion_icons.write().unwrap().insert(key, result.clone());
        result
    }
    ...
    pub fn champion_banner(&self, champion_name: &str) -> Option<PathBuf> {
        ...
        let result = files.iter().filter(|f| f.file_stem().map(|s| normalized(&s.to_string_lossy()) == wanted).unwrap_or(false))
            .find(|f| f.extension().map(|e| e == "png").unwrap_or(false))
            .or_else(|| files.iter().find(|f| f.file_stem().map(|s| normalized(&s.to_string_lossy()) == wanted).unwrap_or(false)))
            .cloned()
            .or_else(|| self.champion_icon(champion_name));
        self.champion_banners.write().unwrap().insert(key, result.clone());
        result
    }

    pub fn talent_icon(&self, talent_id: Option<u32>, champion_name: &str, talent_name: &str) -> Option<PathBuf> {
        let key = format!("{}:{}:{}", talent_id.unwrap_or(0), normalized(champion_name), normalized(talent_name));
        ...
        let result = files.iter().filter(|f| f.file_stem().map(|s| normalized(&s.to_string_lossy()) == wanted).unwrap_or(false))
            ...
            .or_else(|| files.iter().find(|f| f.file_stem().map(|s| normalized(&s.to_string_lossy()) == wanted).unwrap_or(false)))
            .cloned();
        self.talent_icons.write().unwrap().insert(key, result.clone());
        result
    }

    pub fn map_image(&self, map_name: &str) -> Option<PathBuf> {
        let cleaned: String = map_name.split_whitespace()
        ...
        let result = files.iter().find(|f| f.file_stem().map(|s| normalized(&s.to_string_lossy()) == normalized(&format!("ranked {}", wanted))).unwrap_or(false))
            .or_else(|| files.iter().find(|f| { let n = normalized(&f.to_string_lossy()); n.contains(&wanted) && n.contains("ranked") }))
            .or_else(|| files.iter().find(|f| normalized(&f.to_string_lossy()).contains(&wanted)))
            .cloned();
        self.map_images.write().unwrap().insert(wanted, result.clone());
        result
    }
    ...
    pub fn rank_icon(&self, tier: u32) -> Option<PathBuf> {
        ...
        let result = if tier == 0 {
            files.iter().find(|f| normalized(&f.to_string_lossy()).contains("rankiconqualifying")).cloned()
        } else if tier >= 27 {
            files.iter().find(|f| normalized(&f.to_string_lossy()).contains("rankicongrandmaster")).cloned()
        } else if tier == 26 {
            files.iter().find(|f| normalized(&f.to_string_lossy()).contains("rankiconmaster")).cloned()
        } else {
            ...
            files.iter().find(|f| f.file_stem().map(|s| normalized(&s.to_string_lossy()) == wanted).unwrap_or(false)).cloned()
        };
        self.rank_icons.write().unwrap().insert(tier, result.clone());
        result
    }
    ...
    pub fn icon(&self, name: &str, preferred_ext: Option<&str>) -> Option<PathBuf> {
        ...
        let result = preferred.as_ref()
            .and_then(|ext| files.iter().find(|f| {
                let name_ok = f.file_stem()
                    .map(|s| normalized(&s.to_string_lossy()) == wanted)
                    .unwrap_or(false);
                let ext_ok = f.extension()
                    .map(|e| e == ext.as_str())
                    .unwrap_or(false);
                name_ok && ext_ok
            }))
    ...
    fn load_champion_files(&self) -> Vec<PathBuf> { self.load_files_with_lock(&self.champion_files, "champions", false) }
    fn load_map_files(&self) -> Vec<PathBuf> { self.load_files_with_lock(&self.map_files, "maps", false) }
    fn load_rank_files(&self) -> Vec<PathBuf> { self.load_files_with_lock(&self.rank_files, "rank-tiers", true) }
    fn load_icon_files(&self) -> Vec<PathBuf> { self.load_files_with_lock(&self.icon_files, "icons", false) }

    fn load_files_with_lock(&self, lock: &Arc<RwLock<Option<Vec<PathBuf>>>>, dir: &str, recursive: bool) -> Vec<PathBuf> {
        if let Some(files) = lock.read().unwrap().as_ref() { return files.clone(); }
    ...
    fn load_files(&self, directory: &str, recursive: bool) -> Vec<PathBuf> {
        ...
        match std::fs::read_dir(&dir) {
            Ok(entries) => entries.filter_map(|e| e.ok()).flat_map(|e| {
                let p = e.path();
                if p.is_file() {
                    if is_image_ext(&p) {
                        vec![p]
        ...
        match std::fs::read_dir(directory) {
            Ok(entries) => {
                for entry in entries.filter_map(|e| e.ok()) {
                    let p = entry.path();
                    if p.is_file() { if is_image_ext(&p) { files.push(p); } }
                    else { files.extend(self.load_nested_files(&p)); }
                }
            }
            Err(_) => {}
        }
